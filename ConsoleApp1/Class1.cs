...
namespace ConsoleApp1
{
    public static class Global
    {
        ...
        public static void Test()
        {
            ...
            // 创建一些测试张量
            Tensor tensor1 = 1 * torch.ones(new long[] { 1, 2 });
            Tensor tensor2 = 2 * torch.ones(new long[] { 1, 2 });
            Tensor tensor3 = 3 * torch.ones(new long[] { 1, 2 });
            ...
            {
                Tensor output = torch.tensor(0), value = torch.tensor(0);

                (output, value) = outputs[i].OutCollection.Take();
